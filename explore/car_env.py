...
class CarEnv(object):
    ...
    def render(self):
        if self.viewer is None:
            self.viewer = Viewer(*self.viewer_xy, self.car_info, self.sensor_info, self.obstacle_coords, self.obstacle_coords1,self.obstacle_coords2)
#            self.viewer = Viewer(*self.viewer_xy, self.car_info, self.sensor_info, self.obstacle_coords)
    ...
    def _update_sensor(self):
        ...
        for si in range(len(self.sensor_info)):
            ...
            for oi in range(len(self.obstacle_coords1)):
                ...
                if np.cross(r, s) != 0:  # may collision
                    ...
                    if 0 <= t <= 1 and 0 <= u <= 1:
                        ...
                        possible_intersections.append(intersection)
                        possible_sensor_distance.append(np.linalg.norm(u*s))
            for oi in range(len(self.obstacle_coords2)):
                p = self.obstacle_coords2[oi]
                r = self.obstacle_coords2[(oi + 1) % len(self.obstacle_coords2)] - self.obstacle_coords2[oi]
                if np.cross(r, s) != 0:  # may collision
                    t = np.cross((q - p), s) / np.cross(r, s)
                    u = np.cross((q - p), r) / np.cross(r, s)
                    if 0 <= t <= 1 and 0 <= u <= 1:
...
class Viewer(pyglet.window.Window):
    ...
    def __init__(self, width, height, car_info, sensor_info, obstacle_coords, obstacle_coords1, obstacle_coords2):
#    def __init__(self, width, height, car_info, sensor_info, obstacle_coords):
        ...
        c = (134, 181, 244) * 13
        self.obstacle = self.batch.add(13, pyglet.gl.GL_LINE_LOOP, background, ('v2f', obstacle_coords.flatten()), ('c3B', c))
        
        c = (134, 181, 244) * 8
        self.obstacle1 = self.batch1.add(8, pyglet.gl.GL_LINE_LOOP, background, ('v2f', obstacle_coords1.flatten()), ('c3B', c))

        c = (134, 181, 244) * 9
        self.obstacle2 = self.batch2.add(9, pyglet.gl.GL_LINE_LOOP, background, ('v2f', obstacle_coords2.flatten()), ('c3B', c))
    def render(self):
        pyglet.clock.tick()
        self._update()
        self.switch_to()
